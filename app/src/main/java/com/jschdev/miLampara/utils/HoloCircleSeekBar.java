...
public class HoloCircleSeekBar extends View {
    ...
    private void initAttributes(TypedArray a) {
        mColorWheelStrokeWidth = a.getInteger(
                R.styleable.HoloCircleSeekBar_wheel_size,
                COLOR_WHEEL_STROKE_WIDTH_DEF_VALUE);
        mPointerRadius = a.getDimension(
                R.styleable.HoloCircleSeekBar_pointer_size,
                POINTER_RADIUS_DEF_VALUE);
        max = a.getInteger(R.styleable.HoloCircleSeekBar_max,
                MAX_POINT_DEF_VALUE);
        ...
        String text_color_attr = a
                .getString(R.styleable.HoloCircleSeekBar_text_color);

        text_size = a.getDimensionPixelSize(
                R.styleable.HoloCircleSeekBar_text_size,
                TEXT_SIZE_DEFAULT_VALUE);

        init_position = a.getInteger(
                R.styleable.HoloCircleSeekBar_init_position, 0);

        start_arc = a.getInteger(R.styleable.HoloCircleSeekBar_start_angle,
                START_ANGLE_DEF_VALUE);
        end_wheel = a.getInteger(R.styleable.HoloCircleSeekBar_end_angle,
                END_WHEEL_DEFAULT_VALUE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // mCircleTextColor);
        float miX = (mColorWheelRectangle.centerX())
                - (textPaint.measureText(text) / 2);
        float miY = mColorWheelRectangle.centerY() + bounds.height() / 2;

        if (show_text)
            canvas.drawText(text, miX, miY, textPaint);

        Paint paint = new Paint();

        paint.setColor(textPaint.getColor());
        paint.setTextSize(textPaint.getTextSize() / 3);
        paint.setTextAlign(Align.CENTER);

        canvas.drawText(subText, mColorWheelRectangle.centerX(),
                miY * 2, paint);
